...
function bothUpwardDownward(id, pieceName, movesId, blackOrWhite) {
    ...
    if (pieceName.startsWith(pawnValue)) {
        ...
        if (
            (id.charAt(0) == "b" && familiarMove.value == "blank") || (id.charAt(0) == "g" && familiarMove.value == "blank")) {
        ...
        if (idInNumber + 1 <= 8 && idInNumber + 1 >= 1 && verticalLimit.includes(slide) && check(getIdAndValue(slide + (idInNumber + 1))) != coinSwap) movesId.push(slide + (idInNumber + 1));
        if (idInNumber - 1 <= 8 && idInNumber - 1 >= 1 && verticalLimit.includes(slide) && check(getIdAndValue(slide + (idInNumber - 1))) != coinSwap) movesId.push(slide + (idInNumber - 1));
        ...
        if (idInNumber <= 8 && idInNumber >= 1 && verticalLimit.includes(slide) && check(getIdAndValue(slide + idInNumber)) != coinSwap) movesId.push(slide + idInNumber);
        slide = String.fromCharCode(id.charAt(0).charCodeAt(0) + 1);
        console.log("checking right moves " + slide + idInNumber + " ----slide can vary");
        if (idInNumber <= 8 && idInNumber >= 1 && verticalLimit.includes(slide) && check(getIdAndValue(slide + idInNumber)) != coinSwap) movesId.push(slide + idInNumber);
        idInNumber = Number(id.charAt(1)) - 2;
        ...
        // if(idInNumber+1<=max_move && idInNumber+1>=min_move && (check(getIdAndValue(slide+(idInNumber+1)))=="blank"||check(getIdAndValue(slide+(idInNumber+1)))!=coinSwap))movesId.push(slide+(idInNumber+1));
        // if(idInNumber-1<=max_move && idInNumber-1>=min_move && (check(getIdAndValue(slide+(idInNumber-1)))=="blank"||check(getIdAndValue(slide+(idInNumber-1)))!=coinSwap))movesId.push(slide+(idInNumber-1));
        // slide = String.fromCharCode(id.charAt(0).charCodeAt(0)+1);
        // if(verticalLimit.includes(slide) && (check(getIdAndValue(slide+(idInNumber)))=="blank"||check(getIdAndValue(slide+(idInNumber)))!=coinSwap))movesId.push(slide+(idInNumber));
        // slide = String.fromCharCode(id.charAt(0).charCodeAt(0)-1);
        // if(verticalLimit.includes(slide) && (check(getIdAndValue(slide+(idInNumber)))=="blank"||check(getIdAndValue(slide+(idInNumber)))!=coinSwap))movesId.push(slide+(idInNumber));
    }
    return movesId;
